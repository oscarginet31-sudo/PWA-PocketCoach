...
PocketCoach est une Progressive Web App (PWA) conçue pour répondre aux contraintes spécifiques de la pratique sportive en autonomie. Son architecture "Offline-First" garantit une fiabilité totale, permettant une utilisation fluide même dans les infrastructures sportives dépourvues de couverture réseau (zones blanches).
...
Installation (Pre-Cache) : Lors de la première visite, l'intégralité des fichiers statiques (HTML, CSS, JS, Polices) est téléchargée et stockée dans le cache du navigateur.

Interception (Fetch) : Chaque requête réseau est interceptée. Le Service Worker vérifie d'abord la présence du fichier en cache avant de solliciter le réseau.

Cycle de Vie (Update) : À chaque modification du code, le numéro de version (constante VERSION) est incrémenté. Lors de l'activation, un script de nettoyage purge automatiquement les anciens caches pour garantir que l'utilisateur dispose toujours de la dernière version.
...
Enregistrement de séance : Création d'une page dédiée pour historiser les séances en fonction de la date.
